...
async function main() {
    ...
    console.log("signedTxArray[1] ", signedTxArray[1])

    // directly stringify wrong. can't capture signature map
    // let txString = JSON.stringify(signedTxArray[1])
    // console.log('txString ', txString)
    let txString2 = safeTransactionToString(signedTxArray[1])
    console.log('txString2 ', txString2)
    ...
    let txNorm = JSON.parse(txString2)
    console.log('txNorm ', txNorm)
    console.log('-------- ')

    // if (balance1 > 0 || balance2 > 0) {
    let signedTx2 = await genMultisigSignedTx(signer2, txNorm, safeAddress);
