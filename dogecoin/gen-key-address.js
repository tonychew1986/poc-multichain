...
var acct = root.derivePath("m/44'/3'/0'");
...
const child2 = root.derivePath("m/44'/3'/0'/0/1")
const child3 = root.derivePath("m/44'/3'/0'/0/2")
...
data = data.slice(4);
data = Buffer.concat([Buffer.from('0x043587cf','hex'), data]); // see https://lists.linuxfoundation.org/pipermail/bitcoin-dev/2017-September/014907.html for tenative (non-BIP-official) version bytes
let dgub = b58.encode(data);
console.log("dgub ", dgub);

const child1Address = bitcoin.payments.p2pkh({ pubkey: child1.publicKey, network: DOGECOIN }).address
const child2Address = bitcoin.payments.p2pkh({ pubkey: child2.publicKey, network: DOGECOIN }).address
const child3Address = bitcoin.payments.p2pkh({ pubkey: child3.publicKey, network: DOGECOIN }).address
